...
setTimeout(function () {
    ...
    chrome.cookies.set({
        url: bingUrl, domain: '.bing.com', name: '_NTPC', value: !localStorage["pc"] ? defaultPC : localStorage["pc"], sameSite: 'no_restriction', secure: true
    }, function (cookie) {
...
function SendPingDetails(status) {
    ...
    setTimeout(function () {
        var pc = (localStorage.pc == undefined || localStorage.pc == "" || localStorage.pc == null) ? "BG02" : localStorage.pc;
...
chrome.webRequest.onBeforeRequest.addListener(function (details) {
    ...
    function getSearchUrl(queryString, pc) {
        return "https://www.bing.com/search?EID=MBSC&form=BGGCDF&pc=" + pc + "&q=" + queryString;
    }
    ...
    var URL = getSearchUrl(queryString, partnerCode);

    return {
        redirectUrl: URL
    };
}, {
    urls: ["*://www.bing.com/search?EID=MBSC&form=BGGCMF&pc=BG02*"] // List of URLs
